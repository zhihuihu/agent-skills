...
async function discoverSpec(scriptDirectory = path.dirname(fileURLToPath(import.meta.url))) {
  const checked = [];
...
var OpenAPIExplorer = class _OpenAPIExplorer {
  static async create(spec = null, { depth = 3, limit = 50 } = {}) {
    const explorer = new _OpenAPIExplorer(depth, limit);
  ...
  async info() {
    ...
    const securitySchemes = this.version.startsWith("OpenAPI") ? this.data.components?.securitySchemes || {} : this.data.securityDefinitions || {};
    return {
      source: this.source,
  ...
  async tags() {
    ...
    const names = [.../* @__PURE__ */ new Set([...descriptions.keys(), ...counts.keys()])].sort((a, b) => a.localeCompare(b));
    return {
      source: this.source,
      tags: names.map((name) => ({
        name,
  ...
  async search(query, { tag = null, method = null } = {}) {
    ...
    for (const { apiPath, method: itemMethod, operation, pathItem } of await this.operations()) {
      ...
      const fields = [
        ...
        ...tags,
        JSON.stringify(pathItem.parameters || []),
  ...
  schemaRegistry() {
    return this.version.startsWith("OpenAPI") ? this.data.components?.schemas || {} : this.data.definitions || {};
  }
  async schema(name) {
    const schemas = this.schemaRegistry();
    ...
    const outline = await this.schemaOutline(schemas[actualName], this.source, this.depth, /* @__PURE__ */ new Set([identity]));
  ...
  async schemaOutline(schema, source, depth, seen) {
    ...
    if (schema.$ref) {
      ...
      const expanded = await this.schemaOutline(resolved, resolvedSource, depth - 1, /* @__PURE__ */ new Set([...seen, identity]));
    ...
    if (Object.hasOwn(schema, "items")) {
      result.items = depth ? await this.schemaOutline(schema.items, source, depth - 1, seen) : { summary: schemaBrief(schema.items) };
    }
    if (Object.hasOwn(schema, "additionalProperties")) {
      const value = schema.additionalProperties;
      result.additionalProperties = isObject(value) && depth ? await this.schemaOutline(value, source, depth - 1, seen) : value;
    }
    if (isObject(schema.properties)) {
      result.properties = {};
      for (const [key, value] of Object.entries(schema.properties)) {
        result.properties[key] = depth ? await this.schemaOutline(value, source, depth - 1, seen) : { summary: schemaBrief(value) };
      }
    }
    for (const keyword of ["allOf", "oneOf", "anyOf"]) {
      if (Array.isArray(schema[keyword])) {
        ...
        for (const value of schema[keyword]) {
          result[keyword].push(depth ? await this.schemaOutline(value, source, depth - 1, seen) : { summary: schemaBrief(value) });
        }
      }
    }
...
function renderText(command, data) {
  ...
  if (command === "tags") {
    return [`\u6587\u6863 ${data.source} \u4E2D\u7684 Tag\uFF1A`, ...data.tags.map(
      (item) => `- ${item.name} (${item.operationCount})${item.description ? `: ${item.description}` : ""}`
    )].join("\n");
  }
...
var HELP = `\u7528\u6CD5\uFF1Aopenapi-explorer [\u5168\u5C40\u9009\u9879] <\u547D\u4EE4> [\u547D\u4EE4\u9009\u9879\u53CA\u53C2\u6570]
...
function parseCli(argv) {
  ...
  const commands = /* @__PURE__ */ new Set(["info", "tags", "list", "search", "operation", "schema"]);
...
var invokedAsScript = process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url);
